...
var ulBtn=document.querySelector("#ul-btn-bottom")
        function liIndex(numoRoTarget){
            let num;
            if(typeof numoRoTarget=='number'){
                num = numoRoTarget
            }else{
                let node = numoRoTarget.target;
                if(node.nodeName=='LI'){
                    num = node.innerText
                    mySwiper.slideTo(node.innerText-1, 1000, false);
                }else{
                    return
                }
            }
            Array.from(ulBtn.children).forEach(ele=>{
                ele.className='';
        ...
        var mySwiper = new Swiper('#lunbo', {
                ...
                on:{
                    slideChangeTransitionStart(){
                        //得到序号   当长度等于  6
                        let swiperIndex=this.activeIndex>5 ? 1 :this.activeIndex;
                         liIndex(swiperIndex)
                    }
                } 
            })
        
        ulBtn.addEventListener("click",liIndex)